...
# --- Users OU (idempotent: create each OU separately, continue on failure) ---
if ! ldapsearch -x -H "ldap://${LDAP_HOST}:${LDAP_PORT}" -D "${LDAP_ADMIN}" -w "${LDAP_PASS}" "uid=ldap_user_1,ou=users,dc=luban-cae,dc=com" uid 2>/dev/null | grep -q "uid=ldap_user_1"; then
    ...
    # Create each OU independently so a failure on one does not stop the script
    ldapadd -x -H "ldap://${LDAP_HOST}:${LDAP_PORT}" -D "${LDAP_ADMIN}" -w "${LDAP_PASS}" <<'OUEOF' 2>/dev/null || true
...
ou: groups
OUEOF
    echo "[LDAP INIT] Organizational units created."
else
...
for group_dn in \
    ...
    if ldapsearch -x -H "ldap://${LDAP_HOST}:${LDAP_PORT}" -D "${LDAP_ADMIN}" -w "${LDAP_PASS}" -b "$group_dn" "(objectClass=*)" 1 /dev/null 2>&1 | grep -q "^dn: $group_dn"; then
